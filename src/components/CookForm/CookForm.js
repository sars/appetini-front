...
@reduxForm(
  {
    form: 'cookForm',
    fields: ['id', 'main_photo_temp_image_id', 'other_photos_temp_image_ids', 'removing_other_photos', 'disabled',
      'removing_sanitary_book_photos', 'sanitary_book_photos', 'sanitary_book_photos_temp_image_ids', 'first_name',
      'last_name', 'first_name_genitive', 'last_name_genitive', 'other_photos', 'main_photo', 'location_attributes',
      'location', 'user.id', 'user.phone', 'user.email', 'user.password', 'user.facebook', 'user.vkontakte', 'fee_percent']
  }, null, {showToast}
)
  ...
  render() {
    ...
    return (
      <form className={styles.root} onSubmit={handleSubmit}>
        ...
        <div className={styles.section}>
          <h3>Отключение кулинара</h3>
          <Checkbox {...fields.disabled} label="Отключен" />
        </div>
